...
import { CreateCarouselItemDto, apiClient } from '../services/api';
import { toast } from 'sonner';

interface AddCarouselItemProps {
  onClose: () => void;
  onSave: (newItem: CreateCarouselItemDto) => void;
  maxOrder?: number;
}

export function AddCarouselItem({ onClose, onSave, maxOrder = 0 }: AddCarouselItemProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newItem: CreateCarouselItemDto = {
    ...
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-card rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-border">
        <div className="sticky top-0 bg-card border-b border-border px-6 py-4 flex items-center justify-between">
          <h2 className="text-2xl font-semibold">Add Carousel Item</h2>
          <button
            onClick={onClose}
